...
if [ -z "${TETRISNEWSEMAILING_HOSTNAME}" ]; then
  TETRISNEWSEMAILING_HOSTNAME=mail.example.com
fi
...
if [ -z "${CHECK_MYHOSTNAME}" ]; then
  echo >> /etc/postfix/main.cf
  echo -e "\n# User Overrides-configuration" >> /etc/postfix/main.cf
  echo "myhostname = ${TETRISNEWSEMAILING_HOSTNAME}" >> /etc/postfix/main.cf
fi
