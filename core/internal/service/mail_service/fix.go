...
func FixPostfixMainConfig(ctx context.Context) {
	...
	headerChecksContent := ""
	regularHeaderChecks := `if /^\s*Received:.*Authenticated sender.*\(PostTetrisNewsEmailing\)/
/^Received: from .*? \([\w\-.]* \[.*?\]\)(.*|\n.*)\(Authenticated sender: (.+)\)\s+by.+\(PostTetrisNewsEmailing\) with (.*)/
...
`
	...
	err = public.ReadEach(consts.POSTFIX_MAIN_CONF, func(row string, cnt int) bool {
		...
		if strings.HasPrefix(strings.TrimSpace(row), "compatibility_level") {
			...
			lines = append(lines, "mail_name = PostTetrisNewsEmailing\n")
			lines = append(lines, "\n")
