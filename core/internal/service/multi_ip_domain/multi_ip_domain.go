...
func (s *MultiIPDomainService) generateNetworkConfig(ctx context.Context, tx gdb.TX, domain, outboundIP string) (int64, error) {
	...
	networkName := "tetrisnewsemailing-net-" + timestamp
...
func addNewRules(ctx context.Context, dk *docker.DockerAPI, configs []SubnetIP) error {
	...
	for _, config := range configs {
		...
		cmdStr := fmt.Sprintf(
			`chroot /host_root /sbin/iptables -t nat -I POSTROUTING -s %s -j SNAT --to-source %s -m comment --comment "tetrisnewsemailing-outbound"`,
			subnet, outboundIP,
		)
...
// Delete all SNAT rules with the "tetrisnewsemailing-outbound" comment
func deleteOldRules(ctx context.Context, dk *docker.DockerAPI) error {
	...
	comment := "tetrisnewsemailing-outbound"
